"""Chile-specific configuration and CLI for chunk precompute."""
...
_SNAP_M: Final[float] = 1000.0


@dataclass(frozen=True)
class Region:
    """One Chile precompute target and its terrain source configuration."""
...
def _snap_outward(bbox: Bbox, res: float = _SNAP_M) -> Bbox:
    minx, miny, maxx, maxy = bbox
    return (math.floor(minx / res) * res, math.floor(miny / res) * res,
            math.ceil(maxx / res) * res, math.ceil(maxy / res) * res)


def _region_bbox(entry: dtm_alos.RegionArchive) -> Bbox:
    """Reproject a region's lon/lat coverage into its UTM CRS, rounded
    outward to the 1000 m grid."""
    transformer = Transformer.from_crs(
        "EPSG:4326", f"EPSG:{entry.epsg}", always_xy=True)
    west, south, east, north = entry.lonlat_bbox
    corners = [transformer.transform(lon, lat) for lon, lat in (
        (west, south), (west, north), (east, south), (east, north))]
    xs = [c[0] for c in corners]
    ys = [c[1] for c in corners]
    return _snap_outward((min(xs), min(ys), max(xs), max(ys)))


def _region(entry: dtm_alos.RegionArchive) -> Region:
    return Region(entry.name, _region_bbox(entry), f"EPSG:{entry.epsg}",
                 "alos_palsar", dtm_alos.fetch)


REGIONS: tuple[Region, ...] = tuple(_region(entry) for entry in dtm_alos.CATALOG)
