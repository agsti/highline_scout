"""Netherlands-specific configuration and CLI for chunk precompute."""
...
_CRS: Final[str] = "EPSG:28992"


@dataclass(frozen=True)
class Region:
    """One Netherlands precompute target and its terrain source configuration."""
...
# AHN national coverage envelope (EPSG:28992 / RD New), rounded outward to the
# chunk grid.  It is the whole service extent, so it also spans the North Sea,
# Wadden Sea and IJsselmeer; those chunks return an all-nodata raster (or an
# ExtentError for the fully offshore ones) and yield no anchors.
REGIONS: tuple[Region, ...] = (
    Region("netherlands", (10_000, 306_000, 280_000, 619_000), _CRS, "ahn_wcs",
           dtm_ahn.fetch),
)


def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="highliner-etl-chunk-netherlands")
