...
def load_ocean_geometry(crs: str,
    ...
    source_path) per process. Geometrically clips to the target CRS's area of
    use before reprojecting to avoid processing the full global polygon and
    intersecting unwanted ocean basins."""
    ...
    # Geometrically clip to the target CRS's area of use (in its native
    # EPSG:4326 bounds) before reprojecting. This intersects actual geometry
    # (not just filtering rows), ensuring features spanning multiple regions
    # are cut to the target area. Cheaper than clipping after reprojection,
    # and avoids rasterize() walking the full global vertex set.
