...
def _recover_interrupted_promotion(source_dir: Path, backup_dir: Path) -> None:
...
def _promote_source(extracted: Path, source_dir: Path) -> None:
    if not source_dir.exists():
        extracted.replace(source_dir)
        return
    backup_dir = source_dir.with_name(f".{source_dir.name}.backup")
...
def download_sources(raw_dir: Path) -> None:
    ...
    for source, patterns in SOURCE_GLOBS.items():
        ...
        backup_dir = raw_dir / f".{source}.backup"
        with (raw_dir / f".{source}.lock").open("w") as lock:
            fcntl.flock(lock, fcntl.LOCK_EX)
            _recover_interrupted_promotion(source_dir, backup_dir)
            ...
            with tempfile.TemporaryDirectory(
                ...
                _promote_source(extracted, source_dir)
