...
validate_args() {
    local n="$1"  aws_credentials_file="$3"
    if ! [[ "$n" =~ ^[1-9][0-9]*$ ]]; then
...
generate_cloud_init() {
    local secrets_file="$1" aws_credentials_file="$2"
    local secrets_env_b64
    pi_env_b64="$(base64 -w0 secrets.env)"
...
runcmd:
  ...
  - bash -c 'while IFS= read -r line; do [ -n "\$line" ] && echo "export \$line" >> /etc/profile.d/pi-env.sh; done < /root/bootstrap/secrets.env'
HEADER
}
...
main() {
    ...
    local i failures=0
    for ((i = 1; i <= n; i++)); do
        create_server "agent-${i}" "$cloud_init_file" || failures=$((failures + 1))
    done
