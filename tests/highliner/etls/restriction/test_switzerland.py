"""Tests for Switzerland's protected-area adapter."""
...
def test_download_restores_previous_source_when_promotion_fails(
    ...
    previous = source_dir / "previous.dbf"
    previous.write_text("usable previous source")
    ...
    monkeypatch.setattr(
        switzerland, "_download",
        lambda _url, dest: dest.write_bytes(b"archive"),
    )

    def fake_extract(_archive: Path, dest: Path) -> None:
        dest.mkdir()
        (dest / "replacement.shp").write_text("replacement")

    monkeypatch.setattr(switzerland, "_extract_flattened", fake_extract)
    monkeypatch.setattr(
        switzerland, "_has_source",
        lambda path, _patterns: path.name == "extracted",
    )
    real_replace = Path.replace

    def fail_replacement(path: Path, target: Path) -> Path:
        if path.name == "extracted" and Path(target) == source_dir:
    ...
    assert previous.read_text() == "usable previous source"
