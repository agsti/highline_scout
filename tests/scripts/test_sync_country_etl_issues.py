...
def test_apply_holds_the_reconciliation_lock_through_listing_and_creation(
    ...
    def fake_open_issues() -> dict[str, str]:
        events.append("listed")
        return {}

    def fake_create_issue(_title: str) -> str:
        events.append("created")
        return "url"

    monkeypatch.setattr(sync, "_apply_lock", fake_lock)
    monkeypatch.setattr(sync, "_open_issues", fake_open_issues)
    monkeypatch.setattr(sync, "_create_issue", fake_create_issue)
